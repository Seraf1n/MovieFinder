...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val database = FirebaseDatabase.getInstance(
            "https://moviefinder-ec8c8-default-rtdb.europe-west1.firebasedatabase.app/"
        )
        val dbReference = database.getReference("count")
        dbReference.get().addOnCompleteListener {
            var count: Long
            if (it.isSuccessful) {
                count = (it.result?.value ?: 0) as Long
                if (count == 3L) {
                    val alert = AlertDialog.Builder(this)
                        .setTitle("Ales!!!")
                        .setMessage("У вас закончились попытки запуска")
                        .setNeutralButton("Продлить") { dialog, which ->
                            dbReference.setValue(0L)
                        }
                        .show()
                } else {
                    count++
                    dbReference.setValue(count)
                }
            } else {
